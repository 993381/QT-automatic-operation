...
void QSingleInstance::setUniqueString(const QString &unique) {
    uniqeueString = unique;
}
...
bool QSingleInstance::resetInstanceID()
{
	if(d->isRunning || d->isMaster)
		return false;
    const QString appKey = uniqeueString.isEmpty() ? QCoreApplication::applicationName() : uniqeueString;
    d->fullId = appKey;
#ifdef Q_OS_WIN
    ...
    QByteArray hashBase = (/*QCoreApplication::organizationName()*/ "dtk_test" + QLatin1Char('_') + appKey/*QCoreApplication::applicationName()*/).toUtf8();
